...
def refine_role(role: str) -> str:
    ...
    return refined if refined else role  # fall back to original if this fails
...
def search_job_info(role: str, max_results: int = 5) -> list[dict]:
    ...
    response = tavily_client.search(
        ...
        # "advanced" returns the most query-relevant chunks from each page
        # (e.g. the actual skills bullet list), instead of "basic" mode's
        # one generic AI-written summary per URL, which tends to just be
        # the article's intro paragraph.
...
def format_sources_for_prompt(search_results: list[dict]) -> str:
    ...
    for r in search_results:
        # advanced mode can return up to 3 relevant chunks concatenated
        # together, so we need more room than basic mode's short summary
